...
import matplotlib
matplotlib.use('Agg')  # Use non-interactive backend for Flask or non-interactive environments
...
def prepare_training_data(sentiment_df, visualize=True):
    ...
    try:
        # Try converting assuming 'datetime' is in seconds
    ...
    for ticker in stock_tickers:
        ...
        if visualize:
            ...
            plt.tight_layout()
            # Save the figure
            plt.savefig(f'static/{ticker}_price_news_sentiment.png')
            plt.close()
...
def train_model(json_folder_path, model_save_path):
    # Load sentiment data
    ...
    combined_data, max_close_prices = prepare_training_data(sentiment_df, visualize=True)
...
def test_model(ticker_symbol, json_folder_path, model_save_path, optimize=False, image_path=None):
    ...
    # Return evaluation metrics and predicted price
    return {
        'mse': mse,
        'rmse': rmse,
        'mae': mae,
